...
async function getRelatedArticlesFromGFG({query, numResults}: {query: string, numResults: number}): Promise<Article[]> {
...
async function getRelatedArticlesFromWikipedia({query, numResults}: {query: string, numResults: number}): Promise<Article[]> {
...
async function getRelatedVideosFromYouTube({query, numResults}: {query: string, numResults: number}): Promise<Video[]> {
  ...
  try {
    ...
    if (matches) {
      ...
      const videoData = data.contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer.contents[0].itemSectionRenderer.contents;
...
const getRelatedArticlesFromGFGfunctionDeclaration : FunctionDeclaration= {
  ...
  async function main() {
    ...
    try {
        ...
        const prompt = "Give me 2 articles and 3 videos related to the topic of 'Data Structures and Algorithms' from GeeksforGeeks, Wikipedia and YouTube.";
